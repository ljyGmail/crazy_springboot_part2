...
public interface StudentDao extends CrudRepository<Student, Integer> {
    ...
    // 定义该方法使用JPQL语句查询
    @Query("select a from Student a where a.clazz.name like ?1")
    ...
    // 本查询方法使用JPQL语句，只查询指定列(投影)
    // 多列以Map返回，列别名将作为key
    @Query("select s.name as nm, s.gender as gender from Student s where s.clazz.name like ?1")
    List<Map<String, Object>> findNameAndGenderByClazzNameJPQL4(String clazzName);

    // 使用命名参数
    @Query("select s.name, s.gender from Student s where s.clazz.name like :clazz_name")
    List<String[]> findNameAndGenderByClazzNameJPQL5(@Param("clazz_name") String clazzName);
    ...
    // 使用命名JPQL查询
    List<Student> namedJpql(String classNamePattern, char gender);
