...
public class UserController {
    ...
    public Flux<User> testFindByAgeBetween(@PathVariable int start, @PathVariable int end) {
        return userDao.findByAgeBetween(start, end);
    }

    @GetMapping("/subName={subName}/passPrefix={passPrefix}")
    public Flux<User> testFindByNameContainsAndPasswordStartsWith(@PathVariable String subName, @PathVariable String passPrefix) {
        return userDao.findByNameContainsAndPasswordStartsWith(subName, passPrefix);
    }
    ...
    public Flux<User> testFindBySql(@PathVariable String namePattern, @PathVariable int minAge) {
        return userDao.findBySql(namePattern, minAge);
    }

    @PutMapping("/{id}/name={name}")
//    @Transactional
    public Mono<Integer> testUpdateNameById(@PathVariable Integer id, @PathVariable String name) {
        return userDao.updateNameById(name, id);
    }

    @GetMapping("/startAge={startAge}/endAge={endAge}")
    public Flux<User> testCustomQuery1(@PathVariable int startAge, @PathVariable int endAge) {
        return userDao.customQuery1(startAge, endAge);
    }

    @GetMapping("/startAge={startAge}/passPattern={passPattern}")
    public Flux<User> testCustomQuery2(@PathVariable int startAge, @PathVariable String passPattern) {
