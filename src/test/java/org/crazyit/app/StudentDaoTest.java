...
public class StudentDaoTest {
    ...
    public void testFindByClazzNameLike(int page, int size) {
        ...
        studentDao.findByClazzNameLike("%Java%", pageable).forEach(System.out::println);
    }

    @ParameterizedTest
    @CsvSource({"女, 20,  25, %基础班", "女, 18, 20, %训练营"})
    public void testFindByGenderAndAgeBetweenOrClazzNameLike(char gender, int start, int end, String clazzPattern) {
        studentDao.findByGenderAndAgeBetweenOrClazzNameLike(gender, start, end, clazzPattern).forEach(System.out::println);
    }
}
